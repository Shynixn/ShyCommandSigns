...
class ShyCommandSignsPlugin : JavaPlugin(), CoroutinePlugin {
    ...
    override fun onEnable() {
        ...
        if (!Version.serverVersion.isCompatible(*versions.toTypedArray())) {
            ...
            logger.log(Level.SEVERE, "ShyCommandSign does not support your server version")
        ...
        if (mcCoroutineConfiguration.isFoliaLoaded && !checkIfFoliaIsLoadable()) {
            logger.log(Level.SEVERE, "================================================")
            logger.log(Level.SEVERE, "ShyCommandSign for Folia requires ShyCommandSign-Premium-Folia.jar")
